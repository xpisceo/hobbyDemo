...
char szTitle[MAX_LOADSTRING] = "网关服务器";					// 标题栏文本
char szWindowClass[MAX_LOADSTRING] = "WeGateServer";			// 主窗口类名
                     ...
                     int       nCmdShow)
{
	...
	if( !theServer.Init() )
	{
		::MessageBox( s_Hwnd, "启动服务器失败", "启动服务器失败", 0 );
		return 0;
	}
	...
	while ( true )
	{
		...
		else
		{
			...
			if( time2 > 1000 )
			{
				...
				char tmp[256];
				::sprintf( tmp, "%s%d FPS=%d", szTitle, theServer.GetGateServerId(), s_FPS );
...
LRESULT CALLBACK EditCmdProc(HWND hWnd, UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	if(iMessage == WM_KEYDOWN && wParam == VK_RETURN) // 回车
	{
		...
		theServer.OnInputCommand( szCmd );
...
namespace We
{
	void LogInfo( const char* format, ... )
	{
		...
		::strcat( str, "\r\n" );
		::SendMessage( s_Edit_Info, EM_SETSEL, s_Info.length(), s_Info.length() );
